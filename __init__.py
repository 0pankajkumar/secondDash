...
def getResults(title, companyName, team, profileArchiveStatus, fromDate, toDate):
	...
	for item in rows:
		...
		if 'postingCreatedDate' in item:
			dateForLabel = f"{str(item['postingCreatedDate'].strftime('%b'))} {str(item['postingCreatedDate'].strftime('%Y'))} - {str(item['Posting Owner Name'])}"
			...
			dateForLabel = f"$ {str(item['Posting Owner Name'])}"
